        ...
        try:
            if not self.is_charging:
                ...
                if id and id != self.id:
                    self.id, credit = self.reader.read()
                    self.get_logger().info(f"RFID ID: {self.id}, Text: {credit}")
                    cleaned_string = credit.replace('\0', '')
